  ...
  useEffect(() => {
    ...
    async function refreshChatBadge() {
      try {
        ...
        // Badge chỉ là thông tin bổ sung, không ảnh hưởng trang quản trị.
      }
    }
    ...
    <aside className="flex h-full w-64 shrink-0 flex-col bg-dark-900 text-gray-300">
      <div className="border-b border-white/10 px-6 py-5">
        <Link to="/" className="group flex items-center gap-2" onClick={() => setSidebarOpen(false)}>
          ...
          <div>
            <p className="text-sm font-bold leading-tight text-white transition-colors group-hover:text-brand-300">Tiến Phát</p>
            <p className="text-xs leading-tight text-gray-500">Quản trị hệ thống</p>
          </div>
        </Link>
      </div>
      ...
      <nav className="flex-1 space-y-1 overflow-y-auto px-3 py-4">
        {NAV_ITEMS.map(({ to, label, icon: Icon, end, badge }) => (
          ...
          >
            ...
            {label}
            {badge && chatBadge > 0 ? (
              <span className="ml-auto flex min-w-5 items-center justify-center rounded-full bg-red-500 px-1.5 py-0.5 text-[10px] font-black text-white">
      ...
      <div className="border-t border-white/10 px-3 py-4">
        <Link
          to="/"
          onClick={() => setSidebarOpen(false)}
          className="mb-3 flex w-full items-center gap-3 rounded-lg bg-brand-500/10 px-3 py-2.5 text-sm font-medium text-brand-300 transition-colors hover:bg-brand-500/20 hover:text-white"
        >
          <Globe2 size={18} />
          Xem website chính
        </Link>
    ...
    <div className="flex h-screen overflow-hidden bg-gray-50">
      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
        <div className="flex items-center gap-3 border-b border-gray-200 bg-white px-4 py-3 md:hidden">
          ...
          {chatBadge > 0 && (
            <Link to="/admin/hoi-thoai" className="ml-auto flex items-center gap-1 rounded-full bg-red-50 px-2.5 py-1 text-xs font-bold text-red-600">
              <MessagesSquare size={15} /> {chatBadge > 99 ? '99+' : chatBadge}
            </Link>
          )}
          <Link to="/" className={clsx('rounded-lg p-2 text-brand-600 hover:bg-brand-50', chatBadge === 0 && 'ml-auto')} aria-label="Về website chính">
            <Globe2 size={20} />
          </Link>
        </div>
