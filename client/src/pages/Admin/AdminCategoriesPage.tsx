...
import { AdminTileSizePanel } from '@/components/AdminTileSizePanel';
              ...
              <thead className="border-b border-gray-100 bg-gray-50">
                <tr>
                  {['Tên danh mục', 'Slug', 'Số SP', 'Trạng thái', 'Thứ tự', 'Thao tác'].map((heading) => (
                    <th key={heading} className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-600">{heading}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50">
                {categories.map((category) => {
                  const hasSizes = category.slug === 'gach-op-lat';
                      ...
                      <tr className={clsx('hover:bg-gray-50/50', expanded && 'bg-brand-50/30')}>
                        <td className="px-4 py-3 font-medium text-gray-900">
                          {hasSizes ? (
                            ...
                            >
                              ...
                              <span className="rounded-full bg-brand-100 px-2 py-0.5 text-xs text-brand-700">{tileSizes.length} kích thước</span>
                            </button>
                          ) : category.name}
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex gap-1">
                            <button onClick={() => openEdit(category)} className="rounded-lg p-1.5 text-gray-400 hover:bg-brand-50 hover:text-brand-600" title="Sửa danh mục"><Pencil size={15} /></button>
                            <button onClick={() => setDeleteId(category.id)} className="rounded-lg p-1.5 text-gray-400 hover:bg-red-50 hover:text-red-500" title="Xóa danh mục"><Trash2 size={15} /></button>
                          </div>
                        </td>
                      </tr>
                      {hasSizes && expanded && (
                        <tr>
                          <td colSpan={6} className="bg-gray-50/80 px-4 py-5">
                            <AdminTileSizePanel sizes={tileSizes} onChange={setTileSizes} />
                          </td>
                        </tr>
                      )}
                    </Fragment>
        ...
        <Modal title={editId ? 'Sửa danh mục' : 'Thêm danh mục'} onClose={() => setModalOpen(false)}>
          <div className="space-y-4">
            <div>
              ...
              <input className="field-input" value={form.name} onChange={(event) => setForm((previous) => ({ ...previous, name: event.target.value }))} placeholder="Gạch Ốp Lát" />
            </div>
            <div>
              <label className="mb-1.5 block text-xs font-semibold uppercase text-gray-600">Mô tả</label>
              <textarea className="field-input resize-none" rows={3} value={form.description} onChange={(event) => setForm((previous) => ({ ...previous, description: event.target.value }))} />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="mb-1.5 block text-xs font-semibold uppercase text-gray-600">Thứ tự</label>
                <input type="number" className="field-input" value={form.sortOrder} onChange={(event) => setForm((previous) => ({ ...previous, sortOrder: event.target.value }))} />
              </div>
              <div className="flex items-end pb-1">
                <label className="flex cursor-pointer items-center gap-2">
                  <input type="checkbox" checked={form.isActive} onChange={(event) => setForm((previous) => ({ ...previous, isActive: event.target.checked }))} className="h-4 w-4 accent-brand-600" />
          ...
          <div className="mt-6 flex justify-end gap-3 border-t border-gray-100 pt-4">
            ...
            <button onClick={handleSave} disabled={saving} className="btn-primary py-2 text-sm disabled:opacity-60">{saving ? 'Đang lưu...' : 'Lưu'}</button>
          </div>
        </Modal>
      )}
