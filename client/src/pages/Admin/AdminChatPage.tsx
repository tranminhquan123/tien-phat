...
const INITIAL_STATS: AdminChatStats = {
  total: 0,
  aiActive: 0,
  waiting: 0,
  active: 0,
...
export function AdminChatPage() {
  ...
  const [stats, setStats] = useState<AdminChatStats>(INITIAL_STATS);
  ...
  const loadList = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (!selectedId && listResponse.sessions?.length) {
        setSelectedId(listResponse.sessions[0].id);
      }
    } catch (error) {
  ...
  const loadDetail = useCallback(async (sessionId: string, silent = false) => {
    if (detailPollingRef.current) return;
    ...
    try {
      ...
      if (!silent) toast.error((error as Error).message || 'Không thể tải nội dung hội thoại');
    } finally {
  ...
  async function handleStatus(status: ChatSessionStatus) {
  ...
  const displayName = currentSession?.customerName || selectedListItem?.customerName || 'Khách chưa cung cấp tên';
  ...
  return (
    <div className="flex min-h-0 flex-col gap-4">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-5">
        <StatCard label="Chờ tiếp nhận" value={stats.waiting} icon={Clock3} active={statusFilter === 'WAITING_ADMIN'} onClick={() => setStatusFilter(statusFilter === 'WAITING_ADMIN' ? '' : 'WAITING_ADMIN')} tone="amber" />
        <StatCard label="Đang hỗ trợ" value={stats.active} icon={Headphones} active={statusFilter === 'ADMIN_ACTIVE'} onClick={() => setStatusFilter(statusFilter === 'ADMIN_ACTIVE' ? '' : 'ADMIN_ACTIVE')} tone="blue" />
        <StatCard label="Tin chưa đọc" value={stats.unreadMessages} icon={MessageCircle} active={false} onClick={() => setStatusFilter('WAITING_ADMIN')} tone="red" />
        <StatCard label="AI đang tư vấn" value={stats.aiActive} icon={Bot} active={statusFilter === 'AI_ACTIVE'} onClick={() => setStatusFilter(statusFilter === 'AI_ACTIVE' ? '' : 'AI_ACTIVE')} tone="violet" />
        <StatCard label="Đã đóng" value={stats.closed} icon={CheckCircle2} active={statusFilter === 'CLOSED'} onClick={() => setStatusFilter(statusFilter === 'CLOSED' ? '' : 'CLOSED')} tone="gray" />
      </div>
      ...
      <div className="grid min-h-[680px] flex-1 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm lg:grid-cols-[320px_minmax(0,1fr)] 2xl:grid-cols-[320px_minmax(0,1fr)_300px]">
        <aside className="flex min-h-0 flex-col border-b border-gray-200 lg:border-b-0 lg:border-r">
          <div className="space-y-3 border-b border-gray-100 p-3">
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-xl border border-gray-200 py-2.5 pl-9 pr-3 text-sm outline-none focus:border-brand-400"
                placeholder="Tên, SĐT, kích thước..."
              />
            </div>
            <select
              value={statusFilter}
              onChange={(event) => setStatusFilter(event.target.value as ChatSessionStatus | '')}
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            {loadingList ? (
              ...
              <div className="flex h-52 flex-col items-center justify-center px-6 text-center text-gray-400">
        ...
        <section className="flex min-h-[620px] min-w-0 flex-col bg-gray-50/60">
          {!selectedId ? (
            <div className="flex flex-1 flex-col items-center justify-center text-gray-400">
              ...
              <p className="font-semibold">Chọn một hội thoại để bắt đầu</p>
            </div>
            ...
            <>
              <div className="flex flex-wrap items-center gap-3 border-b border-gray-200 bg-white px-4 py-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-brand-100 font-black text-brand-700">
                  {displayName.charAt(0).toUpperCase()}
                </div>
                <div className="min-w-0 flex-1">
                  <p className="truncate font-bold text-gray-900">{displayName}</p>
                  <div className="mt-0.5 flex flex-wrap items-center gap-2 text-xs text-gray-400">
                    {currentSession.phone && <span>{currentSession.phone}</span>}
                    <span className={clsx('rounded-full px-2 py-0.5 font-semibold', STATUS_META[currentSession.status].className)}>
                      {STATUS_META[currentSession.status].label}
                    </span>
                  </div>
                </div>
                <div className="flex flex-wrap gap-2">
                  {currentSession.status !== 'ADMIN_ACTIVE' && currentSession.status !== 'CLOSED' && (
                    <button type="button" disabled={changingStatus} onClick={() => handleStatus('ADMIN_ACTIVE')} className="btn-primary py-2 text-xs">
                      <Headphones size={15} /> Tiếp nhận
                    </button>
                  )}
                  {currentSession.status === 'CLOSED' ? (
                    <button type="button" disabled={changingStatus} onClick={() => handleStatus('WAITING_ADMIN')} className="btn-outline py-2 text-xs">
                      Mở lại
                    </button>
                  ) : (
                    <button type="button" disabled={changingStatus} onClick={() => handleStatus('CLOSED')} className="btn-outline border-red-200 py-2 text-xs text-red-600 hover:bg-red-50">
                      <XCircle size={15} /> Đóng
                    </button>
                  )}
                </div>
              </div>
              ...
              <div className="border-t border-gray-200 bg-white p-3">
                <form onSubmit={(event) => { event.preventDefault(); void handleSend(); }} className="flex items-end gap-2">
                  <textarea
                    value={reply}
                    ...
                    disabled={currentSession.status === 'CLOSED' || sending}
                    rows={2}
                    maxLength={2000}
                    placeholder={currentSession.status === 'CLOSED' ? 'Mở lại hội thoại để phản hồi' : 'Nhập phản hồi cho khách hàng...'}
                  ...
                  <button
                    type="submit"
                    disabled={!reply.trim() || sending || currentSession.status === 'CLOSED'}
                    className="flex h-[52px] w-[52px] items-center justify-center rounded-xl bg-brand-600 text-white hover:bg-brand-700 disabled:opacity-40"
                  >
        ...
        <aside className="hidden min-h-0 overflow-y-auto border-l border-gray-200 bg-white p-4 2xl:block">
          {currentSession ? (
            ...
            <div className="text-center text-sm text-gray-400">Chưa chọn khách hàng</div>
          )}
        </aside>
      </div>
    </div>
...
function StatCard({
  ...
  active: boolean;
  onClick: () => void;
  tone: 'amber' | 'blue' | 'red' | 'violet' | 'gray';
}) {
  const tones = {
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={clsx(
        'flex items-center gap-3 rounded-xl border bg-white p-3 text-left transition hover:-translate-y-0.5 hover:shadow-sm',
        active ? 'border-brand-400 ring-2 ring-brand-100' : 'border-gray-200'
      )}
    >
      <span className={clsx('flex h-9 w-9 items-center justify-center rounded-lg', tones[tone])}><Icon size={18} /></span>
      <span>
        <strong className="block text-lg leading-none text-gray-900">{value}</strong>
...
function SessionListItem({
  ...
  item: AdminChatSessionListItem;
  selected: boolean;
  onClick: () => void;
}) {
  const name = item.customerName || item.contact?.name || 'Khách chưa cung cấp tên';
  const lastMessage = item.lastMessage?.content || 'Chưa có nội dung';
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={clsx(
        'w-full border-b border-gray-100 px-3 py-3 text-left transition',
        selected ? 'bg-brand-50' : 'hover:bg-gray-50',
        item.unreadCount > 0 && !selected && 'bg-amber-50/40'
      )}
    >
      <div className="flex items-start gap-2.5">
        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gray-100 text-sm font-black text-gray-600">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 line-clamp-2 text-xs leading-4 text-gray-500">{lastMessage}</p>
          <div className="mt-2 flex items-center justify-between gap-2">
            <span className={clsx('rounded-full px-2 py-0.5 text-[10px] font-semibold', STATUS_META[item.status].className)}>
              {STATUS_META[item.status].label}
            </span>
            <span className="text-[10px] text-gray-400">{formatRelativeTime(item.lastMessageAt)}</span>
          </div>
        </div>
      </div>
    </button>
  ...
  if (message.sender === 'SYSTEM') {
    return (
      <div className="flex justify-center">
        <span className="rounded-full bg-gray-200 px-3 py-1 text-center text-[11px] text-gray-500">{message.content}</span>
      </div>
    );
  }
  ...
  return (
    <div className={clsx('flex items-start gap-2', fromAdmin && 'flex-row-reverse')}>
      <div className={clsx(
        ...
        fromAdmin ? 'bg-brand-600 text-white' : fromCustomer ? 'bg-blue-100 text-blue-700' : 'bg-dark-900 text-white'
      )}>
      ...
      <div className={clsx('max-w-[78%]', fromAdmin && 'text-right')}>
        <p className="mb-1 px-1 text-[10px] font-semibold text-gray-400">
          {fromAdmin ? 'Nhân viên Tiến Phát' : fromCustomer ? 'Khách hàng' : 'Trợ lý tự động'}
        </p>
        <div className={clsx(
          'whitespace-pre-line rounded-2xl px-3 py-2.5 text-left text-sm leading-5',
          fromAdmin
            ? 'rounded-tr-md bg-brand-600 text-white'
            : fromCustomer
              ? 'rounded-tl-md border border-blue-100 bg-white text-gray-800 shadow-sm'
              : 'rounded-tl-md border border-gray-200 bg-gray-100 text-gray-700'
        )}>
          {message.content}
        </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-3 gap-2">
        <a href={session.phone ? `tel:${session.phone}` : undefined} className={clsx('flex flex-col items-center gap-1 rounded-xl border p-2 text-xs font-semibold', session.phone ? 'border-green-100 bg-green-50 text-green-700' : 'pointer-events-none border-gray-100 text-gray-300')}>
          <Phone size={17} /> Gọi
        </a>
        <a href={session.phone ? `https://zalo.me/${session.phone.replace(/\D/g, '')}` : undefined} target="_blank" rel="noreferrer" className={clsx('flex flex-col items-center gap-1 rounded-xl border p-2 text-xs font-semibold', session.phone ? 'border-blue-100 bg-blue-50 text-blue-700' : 'pointer-events-none border-gray-100 text-gray-300')}>
          <MessageCircle size={17} /> Zalo
        </a>
        <a href={session.email ? `mailto:${session.email}` : undefined} className={clsx('flex flex-col items-center gap-1 rounded-xl border p-2 text-xs font-semibold', session.email ? 'border-violet-100 bg-violet-50 text-violet-700' : 'pointer-events-none border-gray-100 text-gray-300')}>
          <Mail size={17} /> Email
        </a>
      </div>
      ...
      <div>
        ...
        {tags.length ? (
          <div className="flex flex-wrap gap-2">
            {tags.map((tag) => <span key={tag} className="rounded-full bg-brand-50 px-2.5 py-1 text-xs font-semibold capitalize text-brand-700">{tag}</span>)}
          </div>
        ) : <p className="text-sm text-gray-400">Chưa đủ dữ liệu nhận diện</p>}
      </div>
      ...
      {session.sourcePage && (
        <a href={session.sourcePage} target="_blank" rel="noreferrer" className="flex items-center gap-2 rounded-xl border border-gray-200 p-3 text-xs font-semibold text-gray-600 hover:border-brand-200 hover:text-brand-600">
          <ExternalLink size={15} /> Mở trang khách đã gửi yêu cầu
        </a>
      )}
      ...
      {contact && (
        <div className="rounded-xl border border-gray-200 bg-gray-50 p-3">
          <div className="flex items-center justify-between gap-2">
            <p className="text-xs font-bold uppercase tracking-wide text-gray-500">Liên hệ đã liên kết</p>
            <span className="rounded-full bg-white px-2 py-0.5 text-[10px] font-semibold text-gray-500">{contact.status}</span>
          </div>
