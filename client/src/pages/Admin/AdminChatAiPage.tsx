...
export function AdminChatAiPage() {
  ...
  return (
    <div className="flex min-h-0 flex-col gap-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="rounded-full bg-violet-100 px-2.5 py-1 text-[10px] font-black text-violet-700">AI Admin</span>
          </div>
          <p className="mt-1 text-sm text-gray-400">Tiếp nhận khách, phân tích nhu cầu, gợi ý sản phẩm và soạn phản hồi.</p>
        </div>
      ...
      <div className="grid min-h-[700px] flex-1 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm lg:grid-cols-[310px_minmax(0,1fr)] 2xl:grid-cols-[310px_minmax(0,1fr)_390px]">
        ...
        <section className="flex min-h-[640px] min-w-0 flex-col bg-gray-50/60">
          {!selectedId ? (
            <EmptyConversation />
            ...
            <>
              <div className="flex flex-wrap items-center gap-3 border-b border-gray-200 bg-white px-4 py-3">
                ...
                <button type="button" onClick={() => setShowMobileAi((value) => !value)} className="flex items-center gap-1.5 rounded-lg bg-violet-50 px-3 py-2 text-xs font-bold text-violet-700 2xl:hidden">
                  {showMobileAi ? <X size={15} /> : <Sparkles size={15} />}
                  {showMobileAi ? 'Đóng AI' : 'AI hỗ trợ'}
                </button>
              ...
              {showMobileAi && (
                <div className="max-h-[70vh] overflow-y-auto border-b border-violet-100 bg-white p-3 2xl:hidden">
                  <AdminAiPanel sessionId={session.id} lastMessageAt={session.lastMessageAt} onUseDraft={(draft) => { setReply(draft); setShowMobileAi(false); }} />
                </div>
              )}
              ...
              <div className="border-t border-gray-200 bg-white p-3">
                <form onSubmit={(event) => { event.preventDefault(); void sendReply(); }} className="flex items-end gap-2">
                  <textarea
                    ...
                    rows={2}
                    maxLength={2000}
                    placeholder={session.status === 'CLOSED' ? 'Mở lại hội thoại để phản hồi' : 'Nhập phản hồi hoặc dùng AI soạn bản nháp...'}
                ...
                <p className="mt-1.5 text-[11px] text-gray-400">AI chỉ tạo bản nháp · Admin kiểm tra rồi tự nhấn gửi · Đồng bộ mỗi 5 giây</p>
              </div>
            </>
          ) : null}
        </section>

        <aside className="hidden min-h-0 overflow-y-auto border-l border-gray-200 bg-white p-3 2xl:block">
          {session ? (
            <div className="space-y-3">
              <AdminAiPanel sessionId={session.id} lastMessageAt={session.lastMessageAt} onUseDraft={setReply} />
              <CustomerCard detail={detail} />
            </div>
          ) : <p className="py-8 text-center text-sm text-gray-400">Chưa chọn khách hàng</p>}
        </aside>
      </div>
    </div>
  );
...
function CustomerCard({ detail }: { detail: AdminChatSessionDetail | null }) {
  ...
  return (
    <section className="rounded-2xl border border-gray-200 bg-white p-4">
      <p className="text-xs font-black uppercase tracking-wide text-gray-400">Thông tin khách hàng</p>
      ...
      <div className="mt-4 space-y-2 text-xs">
        ...
        {session.acceptedAt && <InfoRow label="Tiếp nhận" value={new Date(session.acceptedAt).toLocaleString('vi-VN')} />}
      </div>
    </section>
  );
}
